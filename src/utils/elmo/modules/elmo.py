...
class Elmo(torch.nn.Module):
    ...
    def forward(self,    # pylint: disable=arguments-differ
        ...
        if word_inputs is not None:
            ...
            if self._has_cached_vocab and len(original_word_size) > 2:
                ...
                warn("Word inputs were passed to ELMo but it does not have a cached vocab.")
    ...
    def get_default(
            ...
            options_file: str=os.path.join(
                os.path.dirname(os.path.realpath(__file__)), "../../../../../resources/elmo/elmo_options.json"
            ),
            weight_file: str=os.path.join(
                os.path.dirname(os.path.realpath(__file__)), "../../../../../resources/elmo/elmo_weights.hdf5"
            ),
    ) -> 'Elmo':
        """
        A default ELMO embedding for eloquent purposes
        :param num_output_representations: Number of differing output representations you want to return (unless you
               understand this keep it at 1)
        :param options_file: The options file to use
        :param weight_file: The weight file to use
        :return:
        """
        try:
            return Elmo(options_file, weight_file, num_output_representations, dropout=0.0)
        except FileNotFoundError as e:
            error("Could not find the Elmo weights / options files. "
                  "Make sure you have run \"make elmo\" from python root.")
            raise e
...
class _ElmoBiLm(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if requires_grad and vocab_to_cache:
            warn("You are fine tuning ELMo and caching char CNN word vectors. "
    ...
    def create_cached_cnn_embeddings(self, tokens: List[str]) -> None:
        ...
        from elmo.modules.token_embedders.embedding import Embedding  # type: ignore
