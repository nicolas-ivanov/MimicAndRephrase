...
class Batch(Iterable):
    ...
    def as_tensor_dict(self,
        ...
        if verbose:
            info("Padding batch of size %d to lengths %s", len(self.instances), str(padding_lengths))
            info("Getting max lengths from instances")
        instance_padding_lengths = self.get_padding_lengths()
        if verbose:
            info("Instance max lengths: %s", str(instance_padding_lengths))
        ...
        if verbose:
            info("Now actually padding instances to length: %s", str(lengths_to_use))
        for instance in self.instances:
